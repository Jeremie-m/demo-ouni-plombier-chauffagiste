(() => {
  ...
  function render(input = {}) {
    ...
    // Couleurs réelles dérivées du lead (variables CSS) — prioritaires sur data-palette.
    const colors = input.colors || {};
    const cssVars = { '--accent': colors.accent, '--accent-dark': colors.accentDark, '--surface': colors.surface };
    Object.entries(cssVars).forEach(([name, value]) => { if (value) document.documentElement.style.setProperty(name, value); });
